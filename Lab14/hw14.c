...
int main(int argc, char* argv[])
{
    ...
    simParm simParms = {NX, INFINITY,
                        0.0};  /* Holds the simulator parameters */
    ...
    enum modes
    {
        UNDEF = 0, EU = 1, RK2 = 2, RK3 = 3, RK4 = 4
    } mode = UNDEF;
    enum problems
    {
        TANK = 0, SPRING = 1
                                  ...
                                  &option_index)) != -1)
    {
        ...
        switch (rc)
        {
            ...
            case 'f':                   /* final time */
                ...
                break;
            case 'z':                   /* step size */
                simParms.h = strtod(optarg, NULL);
                break;
            case 'd':                   /* dampening value */
                simParms.damp = strtod(optarg, NULL);
                break;
            case '1':                   /* x1 */
                x0[0] = strtod(optarg, NULL);
                break;
            case '2':                   /* x2 */
                x0[1] = strtod(optarg, NULL);
                break;
        ...
        (simParms.h == INFINITY) || (rc))
    {
        ...
        fprintf(stderr,
                "This simulates various system\n"
                "Usage: %s solver problem -step num -fTime num  initCond [%d] [-damp num]\n"
                "Where: solver    = -eu, -rk2, -rk3, or -rk4\n"
                "       problem   = -tank or -spring\n"
                "       -step     = simulation step size\n"
                "       -ftime    = final time\n"
                "       -damp     = spring dampening value, default is 0.0\n"
                "       -x1, -x2,  =initCond = initial conditions 'vector' (%d)\n"
                " e.g.   %s -rk4 -tank -step.15 -ftime 25 -x1 0 -x2 0\n"
                "        Use RK4 to simulate the water tank for 25 seconds at .15 sec/step\n\n",
                argv[0], NX, NX, argv[0]);
