 ...
 **   Modify struct Data in DynamicArrays.h for payload according application
 ...
 ** 02/03/2021 A. Tumbar   - Implemented CreateDArray,PushToDArray,DestroyDArray
 **/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void CreateDArray(DArray* DArrayHead, unsigned int InitialSize) {
    ...
    Based on capacity allocate memory storage, if necessary
       ...
       else Allocate storage space for payload according to struct Data
   ...
   ************************************************************************/
    ...
    if (!InitialSize)
    {
        ...
        // realloc will allocate this ptr
        DArrayHead->Payload = NULL;
    }
    else
    {
        DArrayHead->Payload = malloc(sizeof(Data) * DArrayHead->Capacity);
        if (!DArrayHead->Payload)
        {
            fprintf(stderr, "Failed to allocate memory for DArray Payload\n");
...
unsigned int PushToDArray(DArray* DArrayHead, Data* PayloadPtr) {
    ...
    ************************************************************************/
    ...
    if (DArrayHead->EntriesUsed + 1 > DArrayHead->Capacity)
    {
        ...
        if (!DArrayHead->Payload)
        {
            fprintf(stderr, "Failed to resize memory for DArray Payload\n");
            fprintf(stderr, "Error [%d] %s\n", errno, strerror(errno));
            exit(REALLOC_ERROR);
        }
    }
