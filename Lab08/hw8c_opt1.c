...
{
    printf("Running with realloc() optimization\n");
...
void AddPoint(LinearFit *DataSet, double X, double Y)
{
    ...
    if (++DataSet->NextElement >= DataSet->Size)
    {
        ...
        NewData_X = (double *) realloc(DataSet->Data_X,
                                       sizeof(double) * DataSet->Size);
        NewData_Y = (double *) realloc(DataSet->Data_Y,
                                       sizeof(double) * DataSet->Size);

        /* Check for any errors */
        if ((NULL == NewData_X) || (NULL == NewData_Y))
        {
            fprintf(stderr, "Error: Could not reallocate memory at line %d\n",
                    __LINE__);
            exit(-99);
        }
