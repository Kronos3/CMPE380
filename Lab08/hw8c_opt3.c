...
{
    printf("Running with complex loop-unrolling optimization\n");
...
void CalculateCoefficients(LinearFit *DataSet, double *A, double *B)
{
    ...
    double S_XX = 0.0, S_XX_2 = 0.0;
    double S_XY = 0.0, S_XY_2 = 0.0;
    double S_X = 0.0, S_X_2 = 0.0;
    double S_Y = 0.0, S_Y_2 = 0.0;
    int lcv;

    /* Compute the sums */
    for (lcv = 0; lcv < DataSet->NextElement; lcv += 2)
    {
        S_XX += DataSet->Data_X[lcv] * DataSet->Data_X[lcv];
        S_XX_2 += DataSet->Data_X[lcv + 1] * DataSet->Data_X[lcv + 1];

        S_XY += DataSet->Data_X[lcv] * DataSet->Data_Y[lcv];
        S_XY_2 += DataSet->Data_X[lcv + 1] * DataSet->Data_Y[lcv + 1];

        S_X += DataSet->Data_X[lcv];
        S_X_2 += DataSet->Data_X[lcv + 1];

        S_Y += DataSet->Data_Y[lcv];
        S_Y_2 += DataSet->Data_Y[lcv + 1];
    } /* for() */
    ...
    (*A) = (((DataSet->NextElement * S_XY) - (S_X * S_Y)) /
            ((DataSet->NextElement * S_XX) - (S_X * S_X)));
    (*B) = (((S_XX * S_Y) - (S_XY * S_X)) /
            ((DataSet->NextElement * S_XX) - (S_X * S_X)));
